var fqt = ['Start the quiz','Your measurements','Shape','Tops','Bottoms','Your perfect size'];
...
function processFQ() {
  ...
  var dart = parseInt($('input[name=dart]').val(), 10);
  var sleeve = parseInt($('input[name=sleeve]').val(), 10);
  var torso = parseInt($('input[name=torso]').val(), 10);
  ...
  var length = 0;
  var textFieldsFeet = parseInt($("#feet option:selected").val(), 10);
  var textFieldsInches = parseInt($("#inches option:selected").val(), 10);
  ...
  var pants0 = parseInt($("input[name=pants0]").val(), 10);
  var pants1 = parseInt($("input[name=pants1]").val(), 10);
  var pants2 = parseInt($("input[name=pants2]").val(), 10);
  var pants3 = parseInt($("input[name=pants3]").val(), 10);
  var pants4 = parseInt($("input[name=pants4]").val(), 10);
  ...
  //Normalized dress size is recommended bust size
  ...
  // typical length of tops customer gets
  if (height === "Petite"){
    length -= 3;
  }
  else if (height === "Tall"){
  ...
  //Now account for height in length rec
  // for the short
  if ((textFieldsFeet === 5 && textFieldsInches < 4) || textFieldsInches < 5){
    length -= 3;
  }
  ...
  // for the tall
  if ((textFieldsFeet === 5 && textFieldsInches > 9) || textFieldsFeet > 6){
    length += 3;
  }
  if (textFieldsFeet === 5 && textFieldsInches === 9){
    length += 1;
  }
  //Assign length, and second size length  
  if (length < -1){
  ...
  var invalid_size = function(){
    $(".dress-warning, .details").hide();
    $(".details").eq(1).show();
    $("#bottom-size").empty();
    $("#bottom-size").append(pant_rec);
    $(".hagrid").eq(2).hide();
  };
  if ($("select#cup_size option:selected").index() >= 7 || dress_size > 14) {
    $(".dress-warning, .details").hide();
    $(".hagrid").show();
  }
  else if (dress_size_primary == 32 && length_primary == "Tall") {
    invalid_size();
  }
  else if (dress_size_primary == 32 && cup_size_primary == "C/D" && length_primary == "Petite") {
    invalid_size();
  }
  else if (dress_size_primary == 40 && cup_size_primary == "A/B" && length_primary != "Regular") {
    invalid_size();
  }
  else {
  	$(".hagrid").hide();
  	...
  	*/
  	...
  	var fqa_data = 'fq:'+dress_size+','+bust_size+','+cup_size+','+pant_size+','+dart+','+sleeve+','+torso+','+textFieldsFeet+','+textFieldsInches+','+textFieldsWeight+','+pants0+','+pants1+','+pants2+','+pants3+','+pants4;
  ...
  var rec = dress_size_rec+" "+cup_size_rec+ " "+length_primary;
  var post_data = 'feet='+textFieldsFeet+'&inches='+textFieldsInches+'&weight='+textFieldsWeight+"&typical_length="+height+'&bust_size='+bust_size+'&cup_size='+cup_size+'&dress_size='+dress_size+'&pant_size='+pant_size+'&pants0='+pants0+'&dart='+dart+'&sleeve='+sleeve+'&torso='+torso+'&pants1='+pants1+'&pants2='+pants2+'&pants3='+pants3+'&pants4='+pants4+'&pants-fit='+pants_fit+'&size='+fq_size+'&second_size='+rec;
  var gdoc = 'https://docs.google.com/a/macros/quincyapparel.com/exec?service=AKfycbwf-6oIEtvPBwRElOrqPHSWRczWGaEZh9OAvgIa';
...
$("#fq-1 button.fq-next").click(function(){ // Step 1
    if ($("#dress_size option:selected").val() == "select" || $("#bust_size option:selected").val() == "select" || $("#cup_size option:selected").val() == "select" || $("#feet option:selected").val() == "select" || $("#inches option:selected").val() == "select" || $("#cup_size option:selected").val() == "select" || $("#pant_size option:selected").val() == "select" || $("#weight option:selected").val() == "select") {
		...
		var TIER1_DATA = ['Feet','Inches','Pounds','Bust','Cup','Select Answer'];
